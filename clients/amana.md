...
last_catchup: 2026-05-11T09:59:07Z
---
...
> 2026-05-08 follow-up: Backtests still showed price spikes even with normalisation active (LPs crossing/uncrossing with no fixed base gap triggers normalisation issues). Nikos observed IG lagging CMC on 2026-05-07 14:16 and raised concern about price aggregation with only 2 LPs; Muhammad confirmed consultation cleared use of any LP for price discovery. Decision: Cameron Hughes changed reference prices to CMC_CENTROID only for metals futures, IG demoted to continuity pool failover. A-book pricers bounced 13:55 BST. Post-change pricing confirmed good across SPXFUT, NDXFUT, DOWFUT, XAGFUT, XAUFUT. [permalink](https://mahifx.slack.com/archives/C08T42TMKU3/p1778242297769709)
...
> 2026-05-08 EU CFD indices update: G30EUR (DAX) and UKXGBP (FTSE) test-traded and confirmed live at 13:37 BST. F40EUR, IBXEUR (E35EUR, Princess confirmed symbol mapping in thread), E50EUR subsequently test-traded — IBXEUR hedged to LMAX_CENTROID correctly, F40EUR to JUMP_CENTROID correctly. E50EUR hedger issue: hedger attempted to cover <0.1 lot (below CMC min) generating rejections; Rory halted hedger, asked Princess to close position, then fixed via riskQuantisationOverride override for E50EUR and F40EUR (hybridHedgerCFD1 bounced 15:35 BST). SWICHF and MIBEUR added to dynamicOrderSpeeds but not covariance matrix — same `Cannot use MaxVar-relative quantity` error; removed from instrumentToProfileMappings 13:28 BST, hedger restarted. D30/UK100 confirmed live by Princess at 13:37. Min/increment sizes for all EU/APAC CFD indices requested from Amana at 17:09 BST (hedging venues listed per instrument: G30EUR/UKXGBP/E50EUR/MIBEUR/SWICHF→CMC, F40EUR/AEXEUR→Jump, IBXEUR→LMAX); Maynard replied at 22:15 with a doc of min/step per symbol across CMC, Jump, LMAX — Rory to review and configure. [permalink](https://mahifx.slack.com/archives/C08SYSMP0EB/p1778233003384909)
> 2026-05-10 update: HybridHedgerFutsP1 and HybridHedgerFutssW1 both failing to start — RTYFUT-H and RTYFUT-U still in setup, causing covar matrix error. Nathan removed both from instrumentToProfileMappings and added Local-Future list to pools.filters.list to get pool processes running. [permalink](https://mahifx.slack.com/archives/C08T42TMKU3/p1778394088913639)
> 2026-05-11 update: Nikos asked to begin EU futures test trading (noting DAX futures volume). Rory checking readiness and focusing EU futures indices first; Nikos also queried status of CAC and EURO STOXX 50 from Friday (2026-05-09) testing — Rory said he would check. Rory also sent a direct ask to Amana at 10:38 to confirm hedging venue for EU futures indices before Mahi can proceed. [permalink](https://mahifx.slack.com/archives/C08SYSMP0EB/p1778488846062839) [venue-ask](https://mahifx.slack.com/archives/C08SYSMP0EB/p1778492307082789)
...
> [resolved] 2026-04-29 — XAU FUT mid formation: CMC dominates on tightness, IG often more market-correct
...
> 2026-05-08 resolution: After further XAUFUT-M price spike investigation (backtests showed spikes even with normalisation active — LPs crossing and uncrossing with no fixed gap), Cameron Hughes removed IG from reference price formation entirely. Reference prices now CMC_CENTROID only for metals futures; IG retained in continuity pool for failover only. [permalink](https://mahifx.slack.com/archives/C08T42TMKU3/p1778242297769709)
...
- 2026-05-08 — Alerts setup TODO (Will Carter): send Amana full list of available Mahi alerts; set up a dedicated Slack channel for alerts; advise client they don't necessarily need alerts because Mahi is already monitoring. Google doc linked internally. [permalink](https://mahifx.slack.com/archives/C08T42TMKU3/p1778250624991589)
- 2026-05-11 — Nikos TOB display query: could not see recent XAUFUT-M client trades in TOB plot (Echo links shared). Nathan explained the trades filter (DISTRIBUTION_LDN/FUTURES_P_CLIENTS must be selected); Nikos confirmed resolved. [permalink](https://mahifx.slack.com/archives/C08SYSMP0EB/p1778479144556789)
